"""Закрытый доступ к сайту: вход по логину и паролю, ограничения по IP.
...
и открыт он ровно тем, у кого есть действующая сессия:

- сессия игрока — основной вход: логин и пароль выдаёт администратор,
  самостоятельной регистрации нет (см. app/players.py);
- сессия админки — вход администратора на /admin;
- сессия сайта — запасной вход по ОБЩЕМУ паролю (задаётся в админке или
  переменной SITE_PASSWORD). Он остался для совместимости и на случай
  «учётку не завели, а попасть надо»; пароль админки подходит тоже.

Поверх этого работают ограничения по адресу:

- строгий режим (ip_only) пускает ТОЛЬКО адреса из белого списка — для
  случая «сайт вижу только я со своего адреса»; логин при этом всё равно
  спрашивается;
- чёрный список (ip_banned) не пускает конкретные адреса ВСЕГДА (список
  правится в админке, раздел «Доступ к сайту»).

Белый список больше НЕ пускает без входа: раз у каждого игрока своя
учётка со своим журналом ставок, «свой адрес» не отвечает на вопрос, кто
именно пришёл. Сессию админки баны и строгий режим не затрагивают —
запереть себя снаружи оператор не должен.
"""
import ipaddress
import logging
import threading
...
def ip_blacklist() -> list[str]:
    """Забаненные адреса и подсети (раздел админки «Доступ к сайту»)."""
    def load() -> list[str]:
...
def gate_enabled() -> bool:
    """Закрыт ли сайт. Всегда закрыт: вход по логину и паролю.

    Функция осталась ради истории и интерфейса (админка и страница входа
    показывают состояние защиты), но выбора «открытый сайт» больше нет:
    без входа посетитель не видит ни вилок, ни линии."""
    return True
...
def valid_session(request: Request) -> bool:
    """Есть ли у запроса действующая сессия: игрока, сайта или админки."""
    return bool(verify_session_token(request.cookies.get(COOKIE_NAME))
                or players.has_session(request)
...
def check_request(request: Request) -> tuple[bool, str]:
    ...
    строгий режим и адрес не в списке, "login" — нужен вход."""
    ip = client_ip(request)
    # Бан адреса и строгий режим действуют раньше сессии: выгнать
    # конкретного посетителя нужно уметь, не отбирая учётку, а «сайт
    # только с моего адреса» на то и строгий, что чужую сессию (украли
    # ноутбук, утёк пароль) с чужого адреса он тоже не пускает.
    if ip_banned(ip) and not admin_session(request):
        return False, "banned"
    entries = whitelist()
    if entries and ip_only() and not ip_in_whitelist(ip, entries) \
            and not admin_session(request):
        return False, "ip"
...
def state(request: Request | None = None) -> dict:
    ...
    return {
        ...
        # Сколько учёток игроков заведено: пока ни одной, войти можно
        # только реквизитами администратора, и админке об этом стоит
        # сказать вслух.
        "players_count": db.count_players(),
    }
