...
# Вилки с доходностью выше порога не показываются: реальные прематч-вилки —
# единицы процентов, а «доходность» в десятки процентов почти всегда означает
# ошибку сопоставления рынков или устаревший кэф (такие пишутся в лог).
ARB_MAX_PROFIT_PCT = float(os.getenv("ARB_MAX_PROFIT_PCT", "15"))

# Допуск на расхождение времени начала матча между БК, сек. Матчи одних и
# тех же соперников с разным временем старта (Лига Про играет по несколько
# раз в день) считаются РАЗНЫМИ событиями и в одну вилку не попадают.
START_TS_TOLERANCE = float(os.getenv("START_TS_TOLERANCE", "900"))
...
# лиги — отдельные страницы. Сколько страниц лиг обходить на каждый вид
# спорта: лиги берутся от крупных к мелким (по счётчику событий в меню).
# Больше лиг = больше матчей, но заметно дольше цикл (страница ≈ 15-20 с);
# при слишком длинном цикле кэфы успевают устареть.
