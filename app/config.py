...
# незачем: прематч-кэфы так быстро не двигаются. Роспись обновляется по
# кругу — не чаще, чем раз в BETCITY_EXT_REFRESH секунд на событие, и не
# больше BETCITY_EXT_MAX_REQUESTS запросов за цикл. При дефолтах (10
# запросов по 100 событий) вся линия (~2900 событий) обходится за ~3 цикла,
# трафик — ~14 МБ на цикл. Больше запросов = роспись свежее, но и трафика
# больше; 0 запросов = роспись только из кэша.
