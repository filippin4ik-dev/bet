...
# build_name_canon_map (фаззи-слияние имён) — не дешёвая операция (сотни
# тысяч сравнений строк на полном наборе котировок), а сканер пересчитывает
# вилки после КАЖДОГО обновления отдельной БК (несколько раз за цикл).
# Кэшируем результат и обновляем не чаще, чем раз в это время, сек —
# соответствия имён меняются медленно (не быстрее появления новых
# матчей), частый пересчёт не нужен.
FUZZY_NAME_MAP_REFRESH = float(os.getenv("FUZZY_NAME_MAP_REFRESH", "20"))
