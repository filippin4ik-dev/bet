...
def init_db() -> None:
    with _lock, _connect() as conn:
        ...
        # Список сессий посетителей (site_visitors) больше не ведётся:
        # каждая открытая вкладка опрашивала сервер раз в 5 секунд, и на
        # каждое устройство копилась строка с историей адресов — в админке
        # эта таблица занимала пол-страницы, а в базе росла без всякой
        # пользы для защиты. Кто на сайте, видно по журналу входов
        # игроков; закрывает доступ чёрный список адресов («Доступ к
        # сайту»). Старую таблицу убираем вместе с её данными.
        conn.execute("DROP TABLE IF EXISTS site_visitors")
