...
_SUBJECT_TOKENS = [
    ...
    ("желт", "cards"), ("карт", "cards"),  # жёлтые карты / карты
    ("двойн", "doublefaults"),     # двойные ошибки (теннис)
    ("эйс", "aces"),               # эйсы (теннис)
    ("фол", "fouls"),              # фолы
    ("офсайд", "offsides"),        # офсайды
    ("удар", "shots"),             # удары (в створ / всего)
    ("передач", "assists"),        # голевые передачи
    ("попыт", "tries"),            # попытки (регби)
    ("вброс", "throwins"),         # вбросы аутов
...
_PERIOD_RE = re.compile(
    ...
    r"(тайм|сет|период|четверт|половин|иннинг)")
_PERIOD_ROOT = {
    "тайм": "half", "период": "period", "четверт": "quarter",
    "половин": "half", "иннинг": "inning", "сет": "set",
}
...
def market_scope(text: str | None) -> str:
    ...
    формулировки. Незнакомый текст даёт пустой субъект (основной рынок),
    чтобы не плодить ложные совпадения.
    """
    if not text:
        return ""
    t = text.lower().replace("ё", "е")
    ...
    for m in _PERIOD_RE.finditer(t):
        ...
        # «сет» как ПЕРИОД (1-й сет) — только с порядковым номером; голый
        # «сетов» уходит в предметный токен sets ниже.
        if base:
            tokens.append(f"{base}{num}")

    for needle, token in _SUBJECT_TOKENS:
        if needle in t and token not in tokens:
            # «сет»/«гейм» как ПРЕДМЕТ счёта — только без порядкового
            # префикса (иначе «1-й сет» уже учтён как период выше)
            if token in ("sets", "games") and re.search(
                    r"\d+\s*-?\s*(?:й|го)?\s*" + needle, t):
                continue
            tokens.append(token)
