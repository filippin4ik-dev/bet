...
# «гейм» — только В ЕДИНСТВЕННОМ ЧИСЛЕ («5-й гейм», «1 сет 5 гейм»): это
# КОНКРЕТНЫЙ гейм как отрезок матча (теннис). Множественное «геймов/
# геймы/геймам» (основная единица счёта: «тотал геймов», «фора по
# геймам») НЕ период — его отсекает негативный lookahead (?![а-я]).
# Без этого «исход 1-го сета» и «исход 1-й сет 5-й гейм» давали один и
# тот же scope (set1) и склеивались в один рынок — ложная вилка.
_PERIOD_RE = re.compile(
    r"(\d+)\s*-?\s*(?:й|я|е|ый|ой|ая|ое|го|ой)?\s*"
    r"(тайм|сет|период|четверт|половин|иннинг|карт|гейм(?![а-я]))")
_PERIOD_ROOT = {
...
_PERIOD_TOKEN_RE = re.compile(
    r"^(half|period|quarter|set|map|inning|game)(\d+)$")
