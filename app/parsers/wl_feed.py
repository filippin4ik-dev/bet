...
# Типы линий (idTipEventSrc), которые парсер умеет превращать в рынки, —
# только их и храним из полной росписи (остальное — экзотика без явной
# семантики, хранить её значит зря жечь память на слабом VPS).
# 2/51 — «Исход 1X2» (с ничьей), матч/период целиком: см. winline.py
# (SRC_WINNER_1X2 / SRC_WINNER_1X2_P) — порядок кэфов в v[] для этих src
# ПРЕДПОЛОЖИТЕЛЬНО «П1/X/П2» по аналогии с остальными БК, но НЕ
# верифицирован на живом фиде (парсер защищается sane_1x2_margin).
DECODABLE_SRC = frozenset({1, 2, 3, 4, 6, 7, 15, 51, 61, 71, 151})
