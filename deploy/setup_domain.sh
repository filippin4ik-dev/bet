...
# Не отказ, а предупреждение: DNS расходится по миру не мгновенно, и
# у сервера бывает несколько адресов (в т.ч. IPv6, NAT).
...
# www-поддомен есть далеко не всегда, а без него certbot валит выпуск
# целиком — пробуем с ним и откатываемся на домен без www.
if ! issue -d "$DOMAIN" -d "www.$DOMAIN"; then
  echo "С www не получилось (скорее всего, нет A-записи www) — выпускаю"
  echo "сертификат только для $DOMAIN"
  issue -d "$DOMAIN"
fi
